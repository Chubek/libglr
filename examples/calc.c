 ...
 * calculator. It parses expressions with addition, subtraction,
 * multiplication, and division, respecting operator precedence.
 *
 * Grammar:
 ...
 */
static void
lexer_init (lexer_t *lexer, const char *input)
{
  lexer->input = input;
  lexer->pos = 0;
  lexer_next (lexer);
...
lexer_next (lexer_t *lexer)
{
  ...
  switch (c)
    {
    case '+':
      lexer->current.type = TOK_PLUS;
      break;
    case '-':
      lexer->current.type = TOK_MINUS;
      break;
    case '*':
      lexer->current.type = TOK_MULTIPLY;
      break;
    case '/':
      lexer->current.type = TOK_DIVIDE;
      break;
    case '(':
      lexer->current.type = TOK_LPAREN;
      break;
    case ')':
      lexer->current.type = TOK_RPAREN;
      break;
    default:
      lexer->current.type = TOK_END;
      break;
...
main (int argc, char *argv[])
{
  ...
  int expr
      = glr_grammar_add_symbol (grammar, GLR_SYMBOL_NONTERMINAL, "expression");
  ...
  /* Create productions */
  /* expression -> term { (+|-) term } */
  glr_symbol_t *expr_body[] = { glr_grammar_get_symbol (grammar, term) };
  glr_grammar_add_production (grammar, expr, expr_body, 1);

  /* term -> factor { (*|/) factor } */
  glr_symbol_t *term_body[] = { glr_grammar_get_symbol (grammar, factor) };
  glr_grammar_add_production (grammar, term, term_body, 1);

  /* factor -> NUMBER */
  glr_symbol_t *factor1[] = { glr_grammar_get_symbol (grammar, tok_number) };
  glr_grammar_add_production (grammar, factor, factor1, 1);

  /* factor -> ( expression ) */
  glr_symbol_t *factor2[] = { glr_grammar_get_symbol (grammar, tok_lparen),
