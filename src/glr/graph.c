                    ...
                    int symbol_id)
{
  ...
  /* Expand edges array */
  glr_graph_edge_t **new_edges = realloc (
      graph->edges, (graph->edge_count + 1) * sizeof (glr_graph_edge_t *));
  if (new_edges == NULL)
    {
      free (edge);
      return NULL;
  ...
  if (from->edge_out_count >= from->edge_out_capacity)
    {
      size_t new_cap
          = from->edge_out_capacity == 0 ? 4 : from->edge_out_capacity * 2;
      glr_graph_edge_t **new_out
          = realloc (from->edges_out, new_cap * sizeof (glr_graph_edge_t *));
      if (new_out == NULL)
        {
          return NULL;
        }
  ...
  if (to->edge_in_count >= to->edge_in_capacity)
    {
      size_t new_cap
          = to->edge_in_capacity == 0 ? 4 : to->edge_in_capacity * 2;
      glr_graph_edge_t **new_in
          = realloc (to->edges_in, new_cap * sizeof (glr_graph_edge_t *));
      if (new_in == NULL)
        {
          return NULL;
        }
