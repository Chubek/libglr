...
glr_forest_node_t *glr_forest_get_node(glr_forest_t *forest,
    ...
    if (position >= forest->node_count) {
        ...
        glr_forest_node_t **new_nodes = realloc(forest->nodes,
                                                new_count * sizeof(glr_forest_node_t *));
        if (new_nodes == NULL) {
            return NULL;
        }
...
int glr_forest_add_child(glr_forest_node_t *parent, glr_forest_node_t *child) {
    if (parent == NULL || child == NULL ||
        parent->type != GLR_NODE_NONTERMINAL) {
        return -1;
    }
    ...
    if (parent->child_count >= parent->capacity) {
        ...
        glr_forest_node_t **new_children = realloc(parent->children,
                                                   new_capacity * sizeof(glr_forest_node_t *));
...
int glr_forest_add_edge(glr_forest_t *forest, glr_forest_edge_t *edge) {
    ...
    if (edge->end_position >= forest->edge_count) {
        ...
        glr_forest_edge_t **new_edges = realloc(forest->edges,
                                                new_count * sizeof(glr_forest_edge_t *));
        if (new_edges == NULL) {
            return -1;
        }
