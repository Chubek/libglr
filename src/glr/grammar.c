...
int glr_grammar_add_symbol(glr_grammar_t *grammar, glr_symbol_type_t type, const char *name) {
    ...
    glr_symbol_t **new_symbols = realloc(grammar->symbols, 
                                          (grammar->symbol_count + 1) * sizeof(glr_symbol_t *));
    if (new_symbols == NULL) {
        free(symbol->name);
        free(symbol);
        return -1;
...
int glr_grammar_add_production(glr_grammar_t *grammar, int head_id,
    ...
    glr_production_t **new_productions = realloc(grammar->productions,
                                                   (grammar->production_count + 1) * sizeof(glr_production_t *));
...
glr_production_t *glr_grammar_get_production(const glr_grammar_t *grammar, int id) {
