 ...
 */
static inline bool glr_fork_has_context(glr_fork_t *fork) {
 ...
 */
static inline void glr_fork_set_context(glr_fork_t *fork, void *context) {
    if (fork != NULL) {
 ...
 */
static inline void *glr_fork_get_context(glr_fork_t *fork) {
    return fork != NULL ? fork->context : NULL;
}
