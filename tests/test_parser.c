...
static void
test_parser_create_destroy (void)
{
...
test_parser_parse_null (void)
{
  ...
  ASSERT_EQ (result.error, GLR_PARSE_ERROR_MEMORY,
             "Should return memory error");
  ASSERT_NULL (result.forest, "Forest should be NULL");
  printf ("PASSED\n");
}

static void
test_parser_user_data (void)
{
