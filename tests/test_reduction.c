...
static void test_item_set_create_destroy(void) {
...
static void test_item_set_add(void) {
    print_test_name("add");
...
static void test_item_set_contains(void) {
    ...
    ASSERT(!glr_item_set_contains(set, different), "Set should not contain different item");
    
    glr_item_set_destroy(set);
    printf("PASSED\n");
}

static void test_item_set_duplicate(void) {
...
static void test_reduction_create_destroy(void) {
    print_test_name("create_destroy");
...
static void test_reduction_helpers(void) {
    print_test_name("helpers");
    
    glr_reduction_t *red = glr_reduction_create(42, 100);
    ASSERT_NOT_NULL(red, "Failed to create reduction");
    
    ASSERT_EQ(glr_reduction_get_production(red), 42, "Should get production ID");
    
    glr_reduction_destroy(red);
    printf("PASSED\n");
}

static void test_item_set_null(void) {
